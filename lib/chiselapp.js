...
chiselApp.controller('chiselCtrl', function($scope, $cookies, chiselChatFactory, ccGuestNames, firebaseFactory, $firebase, userFactory) {
    ...
    $scope.messages = $firebase(firebaseFactory.get('/channels/-JSzeecQWJKzPVPmt5i7/messages'));
    ...
    $scope.deleteMessage = function(messageId, channelId) {
        console.log("Want to delete "+messageId+" from "+channelId);
        var message = firebaseFactory.get('/channels/'+channelId+'/messages/'+messageId);
        if (message) {
            console.log("Removing it");
            message.remove(function (error) { if (error) { console.log ("No removal") } else { console.log ("Nuked it") } });
            return;
        }
