...
chiselApp.controller('chiselCtrl', function($scope, $firebase, $cookies, chiselChatFactory, ccGuestNames, firebaseFactory) {
    ...
    $scope.channels = []; //actual channel content, live FB variables with awesome autosyncing

    $scope.channels[0] = chiselChatFactory.getChannel('');

    $scope.getGuestName = function() {
        return ccGuestNames[Math.round(Math.random() * (ccGuestNames.length - 1))];
    };

    $scope.addMessage = function(e, channelId) {
        if (e.keyCode != 13) return;
        $scope.channels[channelId].messages.$add({
            type : "normal",
